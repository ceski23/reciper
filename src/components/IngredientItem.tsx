...
export const IngredientItem: VFC<Props> = ({ ingredient }) => {
  ...
  const convertedIngredient = !useConversion ? ingredient : convertIngredient(
    ingredient,
    ('unit' in ingredient) ? conversions[ingredient.unit.normalizedName] : undefined,
  );
  const quantity = ('parsed' in convertedIngredient) ? convertedIngredient.quantity.toFixed(useConversion ? precision : 0) : undefined;
  const unitText = ('unit' in convertedIngredient) ? convertedIngredient.unit.formatter.format({
